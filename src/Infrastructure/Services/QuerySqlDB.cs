...
namespace Infrastructure.Services
{
    public class QuerySqlDB<T> : IQuerySqlDB<T> where T : class
    {
        ...
        public virtual async Task<List<T>> GetPaginatedListAsync(Expression<Func<T, bool>> filterExpression, int page, int pageSize, bool tracking = true)
        {
            try
            {
                return tracking ? await _entity.Where(filterExpression).Skip((page - 1) * pageSize).Take(pageSize).ToListAsync() :
                    await _entity.Where(filterExpression).AsNoTracking().Skip((page - 1) * pageSize).Take(pageSize).ToListAsync();
            }
            catch (Exception ex)
            {
                throw new Exception(ex.ToString());
        ...
        public virtual async Task<List<T>> GetPaginatedListIncludeAsync(string navigationPropertyPath, Expression<Func<T, bool>> filterExpression, int page, int pageSize, bool tracking = true)
        {
            try
            {
                return tracking ? await _entity.Include(navigationPropertyPath).Where(filterExpression).Skip((page - 1) * pageSize).Take(pageSize).ToListAsync() :
                    await _entity.Where(filterExpression).AsNoTracking().Skip((page - 1) * pageSize).Take(pageSize).ToListAsync();
            }
            catch (Exception ex)
            {
        ...
        public virtual async Task<int> CountAsync(Expression<Func<T, bool>> filterExpression, CancellationToken cancellationToken = default)
        {
